...
for idx, quote_tag in enumerate(all_quote_tags):
    ...
    tags = []
    # find_all(), for문 사용
    # 1. a태그이면서 class이름이 "tag"인것들 다 찾기
...
# #어쩌고 -> 어쩌고 id인 태그들
html.select(".quote") # quote클래스인 태그들 다 가져와
...
# 반복되는 태그에 copy selector를 하고 마지막부분의 :nth-child(숫자)를 지운다
...
# 명언들만 추출해주세요!
quote_tags = html.select("body > div > div:nth-child(2) > div.col-md-8 > div > span.text")
